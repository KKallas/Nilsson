...
Inputs:
  --path: str — path to tool group dir (e.g. "tools/nilsson/") or workflow dir (e.g. "workflows/daily_report/")
...
def get_repo():
    """Read repo from .nilsson/config.json, fall back to git remote."""
    cfg = PROJECT_DIR / ".nilsson" / "config.json"
    if cfg.exists():
        try:
    ...
    if result.returncode == 0:
        ...
        if m:
            return m.group(1)
    print("Error: could not determine repo. Set 'repo' in .nilsson/config.json.", file=sys.stderr)
    sys.exit(1)


def main():
    parser = argparse.ArgumentParser(description="Publish local tool/workflow as GitHub issue + PR")
    parser.add_argument("--path", required=True, help="Path to publish (e.g. tools/nilsson/ or workflows/daily_report/)")
    ...
    repo = get_repo()
    # Branch name from path: tools/nilsson/foo -> nilsson/pub-nilsson-foo, workflows/bar -> nilsson/pub-bar
    slug = target.replace("/", "-").replace("tools-", "").replace("workflows-", "wf-")
    branch = f"nilsson/pub-{slug}"
