  ...
  try {
    ...
    for (const [group, tools] of Object.entries(groups).sort()) {
      ...
      const groupBtns = isEditing ? [
        nilsson.btn('OK', `saveToolGroupReadme('${group}')`, {cls:'ok'}),
        nilsson.btn('Cancel', `cancelToolGroupEdit()`)
      ] : [
        nilsson.btn('Edit', `editToolGroup('${group}')`),
        nilsson.btn('P', `openPromptGroup('${group}')`, {cls:'small', title:'AI Chat'}),
        nilsson.btn('Copy', `copyToolGroup('${group}')`),
        nilsson.btn('Rename', `renameToolGroup('${group}')`),
        nilsson.btn('Delete', `deleteToolGroup('${group}')`, {cls:'danger'})
      ];

      let bodyHtml = '';
      if (isEditing) bodyHtml += nilsson.readmeEdit('tg-readme-edit', rawReadme);
      else bodyHtml += nilsson.readme(readmeHtml);

      const toolsHtml = tools.map(t => {
        const isToolEditing = _editingTool && _editingTool.group === group && _editingTool.name === t.name;
        const toolBtns = isToolEditing ? [
          nilsson.btn('OK', `saveToolScript('${group}','${t.name}')`, {cls:'ok'}),
          nilsson.btn('Cancel', `cancelToolScriptEdit()`)
        ] : [
          nilsson.btn('Edit', `editToolScript('${group}','${t.name}')`),
          nilsson.btn('P', `openPromptTool('${group}','${t.name}')`, {cls:'small', title:'AI Chat'}),
          nilsson.btn('Copy', `copyToolScript('${group}','${t.name}')`),
          nilsson.btn('Delete', `deleteToolScript('${group}','${t.name}')`, {cls:'danger'})
        ];
        let toolBody = '';
        if (isToolEditing) {
        ...
        return nilsson.item({id: `tool-${group}-${t.name}`, name: t.name, buttons: toolBtns, body: toolBody, origin: t.origin});
      }).join('');

      bodyHtml += nilsson.items(toolsHtml);
      html += nilsson.card({id: `tg-${group}`, name: checkbox + group, meta: `${tools.length} tools`, buttons: groupBtns, body: bodyHtml, cls: isActive ? '' : 'inactive'});
    }
    el.innerHTML = html;
    ...
    el.querySelectorAll('.wf-step-item').forEach(det => {
      det.addEventListener('toggle', async function() {
        ...
        try {
          ...
          if (data.source) inner += nilsson.fold('Script', nilsson.code(data.source));
          if (data.step_template) inner += nilsson.fold('Workflow template', nilsson.code(data.step_template));
          if (data.step_template) inner += nilsson.fold('Testing', buildTestingPanel(tGroup, tName));
...
function buildTestingPanel(group, name) {
  var defaultCtx = '{\n  "previous_results": {},\n  "workflow": "test",\n  "step": "' + name + '"\n}';
  var defaultParams = '{}';
  var runBtn = nilsson.btn('Run', "runToolDebug('" + group + "','" + name + "')", {cls:'primary'});
  return '<div style="display:flex;gap:8px;margin-bottom:8px;">'
    ...
    + '<pre class="nilsson-code" style="max-height:200px;overflow:auto;font-size:11px;"></pre>'
    + '</div>';
}
