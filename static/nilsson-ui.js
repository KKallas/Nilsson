/* nilsson-ui.js — shared UI components
 *
...
const nilsson = {
  ...
  code(src) {
    return `<pre class="nilsson-code">${nilsson._highlight(src)}</pre>`;
  },
  ...
  fold(title, content, opts) {
    ...
    return `<details class="nilsson-fold${cls}"${open}><summary>${title}</summary>${content}</details>`;
  },
  ...
  file(path, src, opts) {
    ...
    return nilsson.fold(`📄 ${path} (${lines} lines)`, nilsson.code(src), {cls});
  },
  ...
  /** Apply syntax highlighting to all .nilsson-code blocks in a container. */
  highlightAll(container) {
    (container || document).querySelectorAll('pre.nilsson-code').forEach(pre => {
      ...
      if (lines.length > 0) {
        lines.forEach(span => {
          ...
          span.innerHTML = nilsson._highlightLine(raw);
        });
      } else {
        const text = pre.textContent;
        pre.innerHTML = nilsson._highlight(text);
      }
    });
  },
  ...
  applyLock(container, dataId, msg) {
    ...
    if (body) body.innerHTML = nilsson.lockOverlay(msg);
  ...
  _highlight(src) {
    return src.split('\n').map(line => {
      return `<span class="line">${nilsson._highlightLine(line)}</span>`;
    }).join('');
  }
};
